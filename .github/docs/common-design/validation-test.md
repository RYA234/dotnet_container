...
### 1.1 エラーチェックの分類とテスト対象

バリデーション設計の3分類に対応してテストを実施する。

| 分類 | テスト対象コンポーネント | 実装層 |
|------|----------------------|--------|
| **単項目チェック** | Data Annotations（[Required], [MaxLength], [Range], [EmailAddress]）| プレゼンテーション層（Controller） |
| **複数項目関連チェック** | Service層の業務ルールバリデーション（複数フィールド組み合わせ） | ビジネス層（Service） |
| **業務タイミングチェック** | Service層のDB参照バリデーション（存在チェック・与信チェック・重複チェック） | ビジネス層（Service） |
| **共通** | ModelState チェック・エラーレスポンス形式 | Controller |
...
1. **単項目チェックの正確性**: 各Data Annotations属性が正しくバリデーションエラーを検出すること
2. **複数項目関連チェックの正確性**: 複数フィールドの組み合わせ検証がService層で正しく動作すること
3. **業務タイミングチェックの正確性**: DB参照を伴うバリデーションが正しい例外をスローすること
4. **ModelState の一貫性**: ModelState.IsValid が false の場合に 400 を返すこと
5. **エラーレスポンスの統一**: バリデーションエラーが標準形式（type, errors, timestamp）で返ること
